...
pub(crate) struct LintPassAudio {
...
impl LintPass for LintPassAudio {
    fn lint(&self, kra_archive: &KraArchive, lint_messages: &mut LintMessages) -> LintPassResult {
        ...
        // Sub-pass #2
        {
            if let Some(file_formats) = self.file_formats.as_ref() {
                ...
                if let Some(kra_audio) = kra_archive.main_doc.image.audio.as_ref() {
                    let kra_audio_path: &str = kra_audio.master_channel_path.value.as_ref();
            ...
            if self.check_missing_files == Some(true) {
                if let Some(kra_audio) = kra_archive.main_doc.image.audio.as_ref() {
                    ...
                    let resolved_source_path = kra_archive
                        ...
                        .join(&kra_audio.master_channel_path.value);
        ...
        // Sub-pass #4
        {
            if self.default_volume == Some(true) {
                if let Some(kra_audio) = kra_archive.main_doc.image.audio.as_ref() {
                    let kra_audio_muted = kra_audio.audio_muted.value;
                    let kra_audio_volume = kra_audio.audio_volume.value;
